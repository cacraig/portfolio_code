<?php
/**
 * Global RestCollectionClustered Model class for Avalara CertExpress.
 * 
 * This class acts defines the default core functionality of the
 * Collections REST API requests for Clustered models. 
 * 
 * Updated: 4/11/13
 * 
...
class RestCollectionClustered extends AppModel {
    public $useTable = false;
    public $name = 'RestCollectionClustered';
    //public $useDbConfig = 'default';
    
    /*Default Attributes*/
    
    // Valid (parameters allowed)
    public $valid_params      = array();
   ...
   */
    public function parse_results(&$data, $limit, $page, $total_pages){
        ...
        */
        ...
        $parse_data = 
            function( $html_view_string, $data, $extension) 
            use (&$parse_data){
                $out = array();
        ...
        */
        ...
        $parse_data_advanced = 
            function($data, &$associations, $extension, $base_url, $lookUp = null, $tgt = null) 
            use (&$parse_data_advanced, &$data_results, $model){
                $out = array();
                foreach ($data as $key => $val){
                    if(is_array($val)){
                        //Recursive Call. 
                        $parse_data_advanced($val, &$associations, $extension, $base_url, $lookUp, $tgt);
                    } else{
                        // Base Case
                        foreach($associations as $as => $af){
                            // This check is kept iterative, and not offloaded to a function
                            // in order to shorten the call stack. 
                            // For more intuitive indexing. certificates -> (taxcode,certificate, buyer, seller)

                        	// Check look up table for model name...if found,
                        	// Then use defined index (as defined in $lookUp)
                        	if(!empty($lookUp[$as])){
                        		$table = $lookUp[$as];
                        	}
                            elseif (preg_match('/[sxz]es$/', $as) OR preg_match('/[^aeioudgkprt]hes$/', $as)) {
                                // Remove "es"
                                $table = substr($as, 0, -2);
                            }
                            ...
                            foreach($af as $x=>$z){
                            	...
                            	if('id' === $x){
                                    $data_results[$data['id']][$table]['href'] = $base_url.$as."/".$data['id'].$extension.'?tgt='.$tgt;
                            	}
                            	
                                $data_results[$data['id']][$table][$x] = $data[$x];
                        	}
                        	// Checks to see if an href is defined for
                        	// all of the foriegn keys. ( includes '_id' )
                        	...
                        	if(!empty($data_results[$data['id']][$model][$table."_id"])
                        	   ...
                        	   $data_results[$data['id']][$table]['href'] = $base_url.$as."/"
                        	                                             .$data_results[$data['id']][$model][$table."_id"]
                        	                                             .$extension.'?tgt='.$tgt;
                        	}
                        }
                    }
        ...
        if(!empty($this->associations)){
            ...
            $parse_data_advanced($data,$this->associations,$this->extension,$this->base_url,$this->lookUp,$this->tgt);
        } else {
